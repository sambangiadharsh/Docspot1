...
const cancelAppointment = async (req, res) => {
  try {
    ...
    if (appointmentData.payment === true && appointmentData.paymentIntent) {
      try {
        const refund = await stripe.refunds.create({
          payment_intent: appointmentData.paymentIntent,
        });

        console.log("Refund Issued:", refund.id);

        // Mark refunded (optional)
        appointmentData.payment = false;
        appointmentData.refund = true; // optional if you add refund field
      } catch (refundErr) {
    ...
    //  Step 2: Mark appointment as cancelled
    appointmentData.cancelled = true;
    await appointmentData.save();

    //  Step 3: Release doctor slot
